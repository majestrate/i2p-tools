...
// run network interface mainloop
  ...
  for {
    ...
    if err == nil {
      ...
      if s == nil {
        ...
        s.gotInetPacket(pkt, link.sendchnl)
      }
    } else {
      // read error
      log.Fatal(err)
...
// we got a link message from i2p
func (link *exitLink) gotMessage(m *linkMessage) {
  // feed it to recv channel
  link.recvchnl <- m
}
    ...
    if err == nil {
      ...
      if err == nil {
        ...
        if err == nil {
          // run server
          go handler.Run()
          // read packets from i2p
          buff := make([]byte, serv.conf.IfMTU + 64)
          var n int
          var addr i2p.I2PAddr
          var msg *linkMessage
          for err == nil {
            // read packet
            ...
            if err == nil {
              ...
              msgbuff := make([]byte, n)
              copy(msgbuff, buff[:n])
              msg, err = parseLinkMessage(addr, msgbuff)
              // tell handler
              handler.gotMessage(msg)
              msg = nil
            }
          }
          s.Close()

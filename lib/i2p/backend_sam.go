...
func (s *samSession) connect(a string) (c net.Conn, err error) {
	...
	if err == nil {
		// send hello
		_, err = fmt.Fprintf(c, "HELLO VERSION MIN=% MAX=%s\n", s.minv, s.maxv)
		if err == nil {
			r := bufio.NewReader(c)
			var line string
			// read hello reply
			...
			if err == nil {
				...
				for sc.Scan() {
					...
					if t == "DEST" || t == "REPLY" {
						...
						// error
						k = nil
						err = errors.New("failed to parse generated keys")
					}
				}
				if err == nil {
					// keys were made
...
func (s *samSession) stream() (ss StreamSession, err error) {
	_, err = fmt.Fprintf(s.c, "SESSION CREATE STYLE=STREAM DESTINATION=%s\n", s.k.priv)
	r := bufio.NewReader(s.c)
	var line string
	line, err = r.ReadString(10)
	if strings.HasPrefix(line, "SESSION STATUS RESULT=OK DESTINATION=") {
